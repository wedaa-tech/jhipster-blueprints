...
class NoteService:
    ...
    async def create_note_service(self, subject: str, description: str) -> Note:
        note = Note(subject=subject, description=description)
        return await self.noteRepository.create_note(note)

    async def get_note_service(self, note_id: int) -> Optional[Note]:
        return await self.noteRepository.get_note(note_id)

    async def get_all_notes_service(self) -> List[Note]:
        return await self.noteRepository.get_all_notes()

    async def delete_note_service(self, note_id: int) -> bool:
