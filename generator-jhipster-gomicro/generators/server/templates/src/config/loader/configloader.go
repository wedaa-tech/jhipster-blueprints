...
type Config struct {
	...
	COMMUNICATION_<%= appServer.baseName.toUpperCase() %> string `yaml:"COMMUNICATION_<%= appServer.baseName.toUpperCase() %>" env:"COMMUNICATION_<%= appServer.baseName.toUpperCase() %>"`
	<%_ })} _%>
}
